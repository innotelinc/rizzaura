...
export function StoreProvider({ children }) {
  ...
  const loadMe = useCallback(async () => {
    try {
      ...
      dispatch({ type: "SET_ME", payload: d.anon ? null : d.user });
    } catch {
      /* server unreachable */
    }
  }, [dispatch]);
  ...
  // real-time: SSE with polling fallback, plus the session
  useEffect(() => {
    loadMe();
    let es = null;
    let iv = null;
    if (typeof EventSource !== "undefined") {
      ...
      es.onmessage = (e) => {
        try {
          ...
          if (evt.type === "snapshot") {
            dispatch({ type: "SYNC", payload: evt.payload });
          } else if (
            evt.type === "aura" ||
            evt.type === "bid" ||
            evt.type === "battle" ||
      ...
      es.onerror = () => {
        // stream dropped — poll until the EventSource reconnects
        if (!iv) {
          iv = setInterval(syncNow, 5000);
          setTimeout(() => {
            if (iv) {
              clearInterval(iv);
              iv = null;
