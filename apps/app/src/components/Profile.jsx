...
const TIER_COLOR = { bronze: "#cd7f32", silver: "#c0c0c0", gold: "#ffd76a", platinum: "#7ff3ff" };

function BadgesPanel({ me }) {
  if (!me) return null;
  const badges = me.badges || [];
  return (
    <div className="card panel">
      <div className="sec-head" style={{ marginBottom: "1rem" }}>
        <div className="tag">Badges & Prestige</div>
        <h2 style={{ fontSize: "1.3rem" }}>
          Your <span className="grad-text">Clout Collection</span>
        </h2>
        <p>
          {me.prestige > 0
            ? `Prestige ${me.prestige} — you've finished ${me.prestige} season(s) in the top 10. 🏛️`
            : "Earn badges by playing. Finish a season top-10 to gain prestige."}
        </p>
      </div>
      {badges.length ? (
        <div className="badge-grid">
          {badges.map((b) => (
            <div key={b.id} className="card badge-card">
              <span className="b-emoji">{b.emoji}</span>
              <span className="b-name">{b.name}</span>
              <span
                className="b-tier"
                style={{ color: TIER_COLOR[b.tier], borderColor: TIER_COLOR[b.tier] }}
              >
                {b.tier.toUpperCase()}
              </span>
        ...
        <div className="empty">
          No badges yet — vote, win battles, and answer census questions to start your collection.
        </div>
      )}
    </div>
  );
